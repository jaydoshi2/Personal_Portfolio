...
export const Carousel = ({ children, className = "" }) => {
  ...
  const contentChild = React.Children.toArray(children).find((child) => child.type === CarouselContent)
...
export const CarouselContent = ({ children, style }) => {
  return (
    <div className="flex transition-transform duration-300 ease-in-out" style={style}>
      {children}
    </div>
  )
}
...
export const CarouselPrevious = ({ className = "", onClick }) => {
  return (
    <button
      onClick={onClick}
      className={`absolute left-2 sm:left-4 top-1/2 transform -translate-y-1/2 z-10 p-1 sm:p-2 rounded-full bg-white/30 hover:bg-white/50 text-white ${className}`}
...
export const CarouselNext = ({ className = "", onClick }) => {
  return (
    <button
      onClick={onClick}
      className={`absolute right-2 sm:right-4 top-1/2 transform -translate-y-1/2 z-10 p-1 sm:p-2 rounded-full bg-white/30 hover:bg-white/50 text-white ${className}`}
